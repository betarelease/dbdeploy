...
public class DbDeploy {
  ...
  public void go() throws Exception {
    ...
    DatabaseSchemaVersionManager databaseSchemaVersion = new DatabaseSchemaVersionManager(deltaset, dbmsSyntax,
        queryExecuter);

    ChangeScriptRepository changeScriptRepository = new ChangeScriptRepository(new DirectoryScanner()
        .getChangeScriptsForDirectory(scriptdirectory));

    ChangeScriptApplier doScriptApplier = new PrintStreamApplier(ApplyMode.DO, new PrintStream(outputfile), dbmsSyntax,
        databaseSchemaVersion);

    ChangeScriptApplier undoScriptApplier = null;

    if (undoOutputfile != null) {
      undoScriptApplier = new PrintStreamApplier(ApplyMode.UNDO, new PrintStream(undoOutputfile), dbmsSyntax,
          databaseSchemaVersion);
